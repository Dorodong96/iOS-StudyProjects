...
class MainViewController: UIViewController {
    ...
    private func bind() { // Alert Action이 tap 되었을 때 (Title, Datetime, 취소)
            ...
            .map { blog -> [BlogListCellData] in
                return blog.documents
                    .map { doc in
                        ...
                        return BlogListCellData(thumbnailURL: thumbnailURL, name: doc.title, title: doc.title, datetime: doc.datetime)
                    }
            }
        ...
        Observable
            .combineLatest(sortedType, cellData) { type, data -> [BlogListCellData] in // 두 데이터의 최신 데이터를 묶어서 전달
                switch type {
                case .title:
