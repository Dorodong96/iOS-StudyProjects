...
class MainViewController: UIViewController {
    ...
    func bind(_ viewModel: MainViewModel) {
        viewModel.cellData
            .drive(tableView.rx.items) { tv, row, data in
                ...
                case 0:
                    ...
                    cell.bind(viewModel.titleTextFieldCellViewModel) // 셀 뷰모델 바인딩
                    return cell
                    
                case 1:
                    let cell = tv.dequeueReusableCell(withIdentifier: "CategorySelectCell", for: IndexPath(row: row, section: 0))
                    
                    cell.selectionStyle = .none // 선택했을 때 회색 음영이 나타나지 않도록
                    cell.textLabel?.text = data
                    cell.accessoryType = .disclosureIndicator // 꺾쇄 표시
                    return cell
        ...
        // view 항목들에 대한 연결
        tableView.rx.itemSelected // 선택이 되었는가에 대해 전달
            .map { $0.row }
...
extension Reactive where Base: MainViewController {
    var setAlert: Binder<Alert> { // Binder로 Alert를 전달받음
